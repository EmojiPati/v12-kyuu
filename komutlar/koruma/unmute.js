...
let mutelirol = db.fetch(`mutelirol.${message.guild.id}`)
  ...
  if (!user) return message.channel.send(embed.setDescription(`${attention} ${message.author}, Eksik arguman kullandınız, \`d!unmute @etiket/ID\``)).then(m => m.delete({ timeout: 7000 }) && message.delete({ timeout: 7000 }))
